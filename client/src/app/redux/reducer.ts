    ...
    public static reduce(oldState: AppState, action: Action): AppState {
        ...
        switch (action.type) {
            ...
            case ActionType.updateCartProduct:
                newState.currentCart.cartProducts[newState.currentCart.cartProducts.findIndex((cp) => cp._id === action.payload.product._id)] = action.payload;
                overallPrice();
                break;
            // Remove Cart Product
            case ActionType.removeCartProduct:
                newState.currentCart.cartProducts.splice(newState.currentCart.cartProducts.findIndex((cp) => cp._id === action.payload), 1);
                overallPrice();
                break; 
            // //Admin
            // Update Product
